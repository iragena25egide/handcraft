...
export default function ProductDetailsPage() {
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <button onClick={() => router.push("/")} className="px-6 py-3 bg-[#0f172a] text-white rounded-xl font-bold text-sm">
          Return Home
        </button>
      </div>
    );
  ...
  const handleAddToCart = () => {
    // We can add multiple quantities by calling addCartItem 'quantity' times, 
    // or we could refactor store.ts to take a quantity. For simplicity, we loop it if it's more than 1.
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-gray-100 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center">
          <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-500 hover:text-[#0f172a] transition-colors font-bold text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-sm font-medium text-gray-400">By <strong className="text-[#0f172a]">{product.artisan}</strong></span>
            </div>

            <div className="flex items-end gap-4 mb-8">
              <span className="text-4xl font-black text-[#0f172a]">RWF {product.price.toLocaleString()}</span>
              {hasDiscount && (
                <span className="text-xl text-gray-400 line-through font-medium mb-1">RWF {product.originalPrice?.toLocaleString()}</span>
              )}
            </div>
            ...
            <div className="border-t border-gray-100 pt-8 mb-8">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-2xl p-2 w-full sm:w-40">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-3 text-gray-400 hover:text-[#0f172a] hover:bg-white rounded-xl transition-colors">
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="font-bold text-[#0f172a] text-lg w-12 text-center">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="p-3 text-gray-400 hover:text-[#0f172a] hover:bg-white rounded-xl transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 bg-gray-50 rounded-[32px] p-6 border border-gray-100">
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-600">Free Delivery<br/>in Kigali</span>
              </div>
              <div className="flex flex-col items-center text-center gap-2">
                <Shield className="w-6 h-6 text-[#0f172a]" />
                <span className="text-xs font-bold text-gray-600">Authentic<br/>Handcrafted</span>
              </div>
              <div className="flex flex-col items-center text-center gap-2">
                <RotateCcw className="w-6 h-6 text-[#0f172a]" />
                <span className="text-xs font-bold text-gray-600">7-Day<br/>Returns</span>
              </div>
            </div>
